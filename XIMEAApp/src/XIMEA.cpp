...
#define XIMEA_MAX_DEVICES 16

const char *XIMEA::driverName = "XIMEA";

static void XIMEAHandleNewImageTaskC(void *drvPvt);


extern "C" {
    /** Configuration command for XIMEA driver; creates a new SeaBreeceSpec object.
...
printf("Before XIMEA_3.inc \n");
#include "XIMEA_3.inc"
printf("After XIMEA_3.inc \n");
/*
...
*/
    ...
    imageThreadId = epicsThreadCreate("XIMEAHandleNewImageTaskC",
                                epicsThreadPriorityMedium,
                                epicsThreadGetStackSize(epicsThreadStackMedium),
                                (EPICSTHREADFUNC)XIMEAHandleNewImageTaskC,
                                this);
    if(imageThreadId == 0) {
...
void XIMEA::report(FILE *fp, int details) {
...
asynStatus XIMEA::initializeDetector() {
    ...
    int   error = 0;                        // XIMEA error result code
    long *device_ids;
    long *serialNumbers;
...
//     #include "XIMEA_3.inc"
 ...
 printf("Before XIMEA_4.inc \n");
	#include "XIMEA_4.inc"
 printf("After XIMEA_4.inc \n");
/*    
...
asynStatus XIMEA::writeInt32(asynUser *pasynUser, epicsInt32 value) {
...
asynStatus XIMEA::readInt32(asynUser *pasynUser, epicsInt32 *value) {
    asynStatus status = asynSuccess;
    int function = pasynUser->reason;
    XI_RETURN XI_status = XI_OK;
    int localvalue = 0;
...
asynStatus XIMEA::writeFloat64(asynUser *pasynUser, epicsFloat64 value) {
    asynStatus status = asynSuccess;
    int function = pasynUser->reason;
    double minimum;
    int error;
...
asynStatus XIMEA::readFloat64(asynUser *pasynUser, epicsFloat64 *value) {
    asynStatus status = asynSuccess;
    int function = pasynUser->reason;
    XI_RETURN XI_status = XI_OK;
...
asynStatus XIMEA::readFloat64Array(asynUser *pasynUser, epicsFloat64 *value, size_t nElements, size_t *nIn)
{
    asynStatus status = asynSuccess;
    int function = pasynUser->reason;
    int pixels;
 ...
 * XIMEAHandleNewImageTask
 */
static void XIMEAHandleNewImageTaskC(void *drvPvt)
{
    XIMEA *pPvt = (XIMEA *)drvPvt;
    
    pPvt->XIMEAHandleNewImageTask();
}
 ...
 * are seen.
 */
void XIMEA::XIMEAHandleNewImageTask(void) {
...
/* uEyeCamConfig */
static const iocshArg XIMEAConfigArg0 = { "Port name", iocshArgString };
static const iocshArg XIMEAConfigArg1 = { "maxBuffers", iocshArgInt };
static const iocshArg XIMEAConfigArg2 = { "maxMemory", iocshArgInt };
static const iocshArg XIMEAConfigArg3 = { "priority", iocshArgInt };
static const iocshArg XIMEAConfigArg4 = { "stackSize", iocshArgInt };
static const iocshArg XIMEAConfigArg5 = { "serial number", iocshArgString };
static const iocshArg * const XIMEAConfigArgs[] = { &XIMEAConfigArg0,
    &XIMEAConfigArg1, &XIMEAConfigArg2, &XIMEAConfigArg3, &XIMEAConfigArg4, &XIMEAConfigArg5 };
    
static const iocshFuncDef configXIMEA = { "XIMEAConfig", 6, XIMEAConfigArgs };

static void configXIMEACallFunc(const iocshArgBuf *args) {
    XIMEAConfig(args[0].sval, args[1].ival, args[2].ival, args[3].ival,
                    args[4].ival, args[5].sval);
}

static void XIMEARegister(void) {
    iocshRegister(&configXIMEA, configXIMEACallFunc);
}

extern "C" {
    epicsExportRegistrar(XIMEARegister);
}
