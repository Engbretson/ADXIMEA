...
#define MQ022HG_MAX_DEVICES 16

const char *MQ022HG::driverName = "MQ022HG";

static void MQ022HGHandleNewImageTaskC(void *drvPvt);


extern "C" {
    /** Configuration command for MQ022HG driver; creates a new SeaBreeceSpec object.
     ...
     *            ASYN_CANBLOCK is set in asynFlags.
     */
    int MQ022HGConfig(const char *portName, int maxBuffers, size_t maxMemory, int priority, int stackSize, const char *id) {
        new MQ022HG(portName, maxBuffers, maxMemory, priority, stackSize, id);
        return (asynSuccess);
    }
    ...
    */
    static void exitCallbackC(void *pPvt){
        MQ022HG *pMQ022HG = (MQ022HG*)pPvt;
        delete pMQ022HG;
    }
}

MQ022HG::MQ022HG(const char *portName, int maxBuffers, size_t maxMemory, int priority, int stackSize, const char *id) :
  //  ADDriver(portName, 1, int(NUM_MQ022HG_PARAMS), maxBuffers, maxMemory, asynDrvUserMask | asynFloat64ArrayMask, 0, ASYN_CANBLOCK, 1, priority, stackSize) {
...
*/
    ...
    imageThreadId = epicsThreadCreate("MQ022HGHandleNewImageTaskC",
                                epicsThreadPriorityMedium,
                                epicsThreadGetStackSize(epicsThreadStackMedium),
                                (EPICSTHREADFUNC)MQ022HGHandleNewImageTaskC,
                                this);
    if(imageThreadId == 0) {
...
void MQ022HG::report(FILE *fp, int details) {
...
asynStatus MQ022HG::initializeDetector() {
    ...
    int   error = 0;                        // MQ022HG error result code
    long *device_ids;
    long *serialNumbers;
...
asynStatus MQ022HG::writeInt32(asynUser *pasynUser, epicsInt32 value) {
...
//		driverName, functionName, function, whoami, value,XI_ERRORS[value]);
   ...
   if(function >= FIRST_MQ022HG_PARAM) 
   {
	  systemInteger = value; 
...
asynStatus MQ022HG::readInt32(asynUser *pasynUser, epicsInt32 *value) {
    asynStatus status = asynSuccess;
    int function = pasynUser->reason;
    XI_RETURN XI_status = XI_OK;
    int localvalue = 0;
...
asynStatus MQ022HG::writeFloat64(asynUser *pasynUser, epicsFloat64 value) {
    asynStatus status = asynSuccess;
    int function = pasynUser->reason;
    double minimum;
    int error;
...
asynStatus MQ022HG::readFloat64(asynUser *pasynUser, epicsFloat64 *value) {
    asynStatus status = asynSuccess;
    int function = pasynUser->reason;
    XI_RETURN XI_status = XI_OK;
...
asynStatus MQ022HG::readFloat64Array(asynUser *pasynUser, epicsFloat64 *value, size_t nElements, size_t *nIn)
{
    asynStatus status = asynSuccess;
    int function = pasynUser->reason;
    int pixels;
...
asynStatus MQ022HG::acquireStart() {
    int imageMode;
    int triggerMode;
...
asynStatus MQ022HG::acquireStop() {
	/*
    asynPrint(pasynUserSelf, ASYN_TRACE_FLOW, "%s:%s Enter\n", driverName, __func__);
 ...
 * MQ022HGHandleNewImageTask
 */
static void MQ022HGHandleNewImageTaskC(void *drvPvt)
{
    MQ022HG *pPvt = (MQ022HG *)drvPvt;
    
    pPvt->MQ022HGHandleNewImageTask();
}
 ...
 * are seen.
 */
void MQ022HG::MQ022HGHandleNewImageTask(void) {
...
/* uEyeCamConfig */
static const iocshArg MQ022HGConfigArg0 = { "Port name", iocshArgString };
static const iocshArg MQ022HGConfigArg1 = { "maxBuffers", iocshArgInt };
static const iocshArg MQ022HGConfigArg2 = { "maxMemory", iocshArgInt };
static const iocshArg MQ022HGConfigArg3 = { "priority", iocshArgInt };
static const iocshArg MQ022HGConfigArg4 = { "stackSize", iocshArgInt };
static const iocshArg MQ022HGConfigArg5 = { "serial number", iocshArgString };
static const iocshArg * const MQ022HGConfigArgs[] = { &MQ022HGConfigArg0,
    &MQ022HGConfigArg1, &MQ022HGConfigArg2, &MQ022HGConfigArg3, &MQ022HGConfigArg4, &MQ022HGConfigArg5 };
    
static const iocshFuncDef configMQ022HG = { "MQ022HGConfig", 6, MQ022HGConfigArgs };

static void configMQ022HGCallFunc(const iocshArgBuf *args) {
    MQ022HGConfig(args[0].sval, args[1].ival, args[2].ival, args[3].ival,
                    args[4].ival, args[5].sval);
}

static void MQ022HGRegister(void) {
    iocshRegister(&configMQ022HG, configMQ022HGCallFunc);
}

extern "C" {
    epicsExportRegistrar(MQ022HGRegister);
}
