...
for node in doneNodes:
    ...
    if node.nodeName in ["Integer", "IntConverter", "IntSwissKnife"]:
        ...
        print '  field(INP,  "@asyn($(PORT),$(ADDR=0),$(TIMEOUT=1))%s")' % nodeName0
        ...
        print '  field(DTYP, "asynInt32")'
        print '  field(OUT,  "@asyn($(PORT),$(ADDR=0),$(TIMEOUT=1))%s")' % nodeName0
        print '  field(DISA, "0")'
        print '}'
        print        
...
#                if (len(name) < 1):
#                name = str(n.getAttribute("DisplayName"))
#                print name
                name = [x for x in elements(n) if str(x.nodeName) == "DisplayName"]
                name = getText(name[0])
