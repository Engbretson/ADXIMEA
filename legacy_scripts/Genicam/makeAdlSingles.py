...
def write_adl_file(fileName):
    ...
    rectangle {
		...
		text {
			...
			textix="%(camera_name)s Features Screen  - $(P)$(R)" #mse
			align="horiz. centered"
		}
    """ %globals())
...
for name, nodes in structure:
    ...
    if (y + boxHeight) > maxScreenHeight:
        ...
        if (w + boxWidth + 5) > maxScreenWidth:
            w += 10
#            write_adl_file(adlFile + str(fileNumber) + ".adl") #mse
            numColumns = 1
            fileNumber += 1
            text = ""
            w = boxWidth
    ...
    for node in nodes:
        ...
        y += 25
    h = max(y, h) + 5; w = w + 10 #mse adjust visual window
    write_adl_file(adlFile + name + ".adl") # mse write single section
    text = ""; h = 40; x = 5 ;y = 40 ; w = boxWidth # mse reset starting dimensions
#   y += 5 #mse
#   h = max(y, h) #mse
