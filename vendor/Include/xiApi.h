...
typedef enum
{
	...
	XI_TRG_SEL_EXPOSURE_ACTIVE   =1, // The trigger controls the start and length of the exposure..
	...
	XI_TRG_SEL_MULTI_SLOPE_PHASE_CHANGE=6, // Selects a trigger controlling the multi slope phase in one Frame (phase0 -> phase1) or (phase1 -> phase2).
	XI_TRG_SEL_ACQUISITION_START =7, // Selects a trigger starting acquisition of first frame.
...
typedef enum
{
	...
	XI_GPO_FRAME_TRIGGER_WAIT    =6, // OOutput is on while camera is ready for trigger
...
typedef enum
{
	XI_TS_RST_ARM_ONCE           =0, // Engine is disabled after time stamp has been reset after selected event.
	XI_TS_RST_ARM_PERSIST        =1, // Engine is armed permanently so each selected event will trigger time stamp reset. 
...
typedef enum
{
	XI_TS_RST_OFF                =0, // No source selected time stamp reset is not armed.
	...
	XI_TS_RST_SRC_SW             =19, // TRIGGER to sensor rising edge is active
